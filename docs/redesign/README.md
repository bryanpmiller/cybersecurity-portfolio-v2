...
| Phase 7 — Selected Projects | Complete | `04_HOMEPAGE_REDESIGN.md` / `05_CASE_STUDY_REDESIGN.md` | Reworked homepage projects into unified featured case-study panels ordered Password Spray, The Buyer, Vulnerability Management, and DISA, preserving title, role, summary, hiring relevance, Problem / Outcome / Concepts context, labeled tools, and case-study CTAs. | None |
...
| Phase 10 — Case Study Pages | Complete | `05_CASE_STUDY_REDESIGN.md` | Reworked the shared project detail experience into professional article-style case studies and refreshed `/projects` into an editorial project index with homepage-aligned case-study cards, numbered review links, hiring relevance, tools, and GitHub evidence actions. Post-final refinement clarifies that Evidence Links lead directly to the GitHub README full technical write-up and supporting artifacts, with consistent Title Case action labels. | Build, lint, type check, route checks, and desktop/mobile browser visual checks passed. |
...
| Phase 13 — Final Review | Complete | `08_QA_FINAL_REVIEW.md` | Completed final senior frontend designer and cybersecurity hiring-manager review. No required fixes remain before acceptance; the verified direct email contact path is now added, the shared resume PDF path targets `bryanpmiller_resume_public.pdf`, and the hero proof row includes a native `DevSecOps pipeline: passing` evidence pill linked to GitHub Actions evidence. | None |
